...
class LambertianModel(object):
    ...
    def __init__(self, diffuse_colour=(.5, .5, .5),
                 light_positions=((.667, -.66, .33),
                                  (-.667, .66, .33))):
        self.diffuse_colour = np.asarray(diffuse_colour)
        self.light_positions = l2_normalize(
            np.asarray(light_positions).reshape(-1, 3), axis=0)
    ...
    def apply(self, mesh, use_texture=False, copy=True):
        ...
        light_directions = l2_normalize(
            self.light_positions.reshape(-1, 1, 3) - mesh.points[None, ...],
            axis=0)
