...
def insert_id_constraint(J, c_id, n_p, n_sites_per_frame, n_frames):
    ...
    J[offset:offset + n_p, :n_p] = np.eye(n_p) * c_id
...
def insert_exp_constraint(J, c_exp, n_p, n_q, n_sites_per_frame, n_frames):
    ...
    size = n_q * n_frames
    exp_const = sp.dia_matrix((np.ones(size) * c_exp, np.array([0])),
                              shape=(size, size))
...
# ---------------------------- LANDMARK JACOBIANS ---------------------------- #
def insert_J_l_p(J, J_l_p, f, n_pixels_per_frame, n_frames):
    si, sj = J_l_p.shape
    i_offset = n_frames * n_pixels_per_frame + f * si
    J[i_offset:(i_offset + si), :sj] = J_l_p


def insert_J_l_q(J, J_l_q, f, n_p, n_pixels_per_frame, n_frames):
    si, sj = J_l_q.shape
    i_offset = n_frames * n_pixels_per_frame + f * si
    j_offset = n_p + f * sj
...
def insert_J_l_c(J, J_l_c, f, n_p, n_q, n_pixels_per_frame, n_frames):
    si, sj = J_l_c.shape
    i_offset = n_frames * n_pixels_per_frame + f * si
    j_offset = n_p + n_frames * n_q + f * sj
...
def insert_frame_to_J(J, j, f, c_l, n_p, n_q, n_pixels_per_frame, n_frames):
    insert_J_f_p(J, j['J_f_p'], f)
    insert_J_f_q(J, j['J_f_q'], f, n_p)
    insert_J_f_c(J, j['J_f_c'], f, n_p, n_q, n_frames)

    c_l_srqt = np.sqrt(c_l)
    insert_J_l_p(J, c_l_srqt * j['J_l_p'], f, n_pixels_per_frame, n_frames)
    insert_J_l_q(J, c_l_srqt * j['J_l_q'], f, n_p, n_pixels_per_frame, n_frames)
    insert_J_l_c(J, c_l_srqt * j['J_l_c'], f, n_p, n_q, n_pixels_per_frame,
                 n_frames)


def insert_frame_to_e(e, j, f, n_sites_per_frame):
    offset = n_sites_per_frame * f
    e[offset:offset + n_sites_per_frame] = np.hstack([j['e_f'], j['e_l']])
